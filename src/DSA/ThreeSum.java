...
//		System.out.println(new ThreeSum().threeSumV2(new int[] {0,0,0,0 }));
		...
		System.out.println(
				new ThreeSum().threeSumV3(new int[] { 2, -3, 0, -2, -5, -5, -4, 1, 2, -2, 2, 0, 2, -4, 5, 5, -10 }));
	}
...
//			System.out.println(main);
			...
			while (left < right) {
				int sum = nums[i] + nums[left] + nums[right];
				System.out.println("main " + i + " =>" + nums[i] + " left " + left + " => " + nums[left] + " right "
						+ right + " => " + nums[right] + " sum " + sum);
				if (sum == 0) {
					list.add(Arrays.asList(nums[i], nums[left], nums[right]));
...
//				break;
	...
	private List<List<Integer>> threeSumV3(int[] nums) {
		List<List<Integer>> list = new ArrayList<>();
		Arrays.sort(nums);
